...
import { AppState, QuizData, UserAnswers, Question } from './types';
...
function App() {
    ...
    try {
      ...
      if (savedData) {
        ...
        if (parsed.activeQuizData && parsed.appState === AppState.QUIZ) {
          ...
          setFileContext(parsed.fileContext || null); // Might be null if we skipped saving it to save space
  ...
  const loadQuizBatch = async (ctx: FileContext, batchIndex: number) => {
  ...
  const handleFileSelect = async (file: File) => {
    setAppState(AppState.PROCESSING);
    setErrorMsg(null);
    localStorage.removeItem(STORAGE_KEY); // Clear old progress on new file
  ...
  const handleBatchChange = async (batchIndex: number) => {
    if (fileContext) {
      await loadQuizBatch(fileContext, batchIndex);
    }
  };

  const handleStartQuiz = (config: { questionCount: number; isRandom: boolean; timeLimit: number }) => {
    ...
    const newActiveData = {
      ...fullQuizData,
      questions: questions,
      timeLimit: config.timeLimit // Store the custom time limit
    };
    ...
    // Initial Save: Use timeLimit as the starting "timeLeft" logic
  ...
  const handleSaveProgress = (answers: UserAnswers, timeLeft: number, overrideData?: QuizData) => {
    try {
      const dataToSave = {
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
      ...
      <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-50 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={handleNewFile}>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {appState === AppState.UPLOAD && (
          <div className="h-full flex flex-col justify-center pt-10">
            <UploadSection onFileSelect={handleFileSelect} isProcessing={false} />
          </div>
        )}
        ...
        {appState === AppState.PROCESSING && (
           <div className="h-[60vh] flex flex-col items-center justify-center">
             ...
             <h2 className="text-2xl font-semibold text-slate-800 dark:text-white mb-2">Đang phân tích tài liệu...</h2>
