...
export const exportToWord = async (data: QuizData, userAnswers: UserAnswers, timeSpent: number, score: number) => {
  ...
  // 2. Score Table
  const correctCount = data.questions.filter(q => userAnswers[q.id] === q.correctAnswer).length;
  // const incorrectCount = data.questions.length - correctCount;
      ...
      children.push(
        new Paragraph({
          children: [
            ...
            new TextRun({ text: q.explanation, color: "1E3A8A", italics: true, size: 20 })
          ],
          indent: { left: 720 },
